...
dataset = dataset.fillna(value=dataset.mean())#fills all the empty numerical spaceswith respective mean of the whole column

                        
                        
(dataset['BsmtFinType2'].fillna(value='VARIOUS',inplace=True))  #fills all the empty character spaces with keyword "various"
...
#changes characters to numericals
...
#((dataset.isnull().sum()))
y = dataset.iloc[:,37].values #selects only home prices          
X = dataset.drop(['SalePrice'],axis=1,inplace=True) #select all columns except home prices


############ to select p values less than 0.05
