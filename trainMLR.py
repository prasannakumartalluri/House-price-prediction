...
dataset = dataset.fillna(value=dataset.mean()) #missing numerical values are replaced by mean
...
"""
scat1= seaborn.regplot(x="MoSold",y="YrSold", data=dataset)
plt.xlabel('MoSold')
plt.ylabel('YrSold')
plt.show()

dataset.drop(['GrLivArea'],axis=1)

print(dataset.columns.get_loc("SalePrice"))
...
"""
stdres = pd.DataFrame(regressor.resid_pearson)
fig=plt.plot(stdres, 'o', ls='None')
l= plt.axhline(y=0,color='r')
plt.ylabel('jdjdj')
plt.xlabel('hddsdjkjksd')
print(fig)

dataclean = dataset.dropna()
print(scipy.stats.pearsonr(dataclean['SalePrice'],dataclean['GrLivArea']))

scat1= seaborn.regplot(x="SalePrice",y="GrLivArea",fit_reg=True, data=dataset)
...
#dataclean = dataset.dropna()
#print(scipy.stats.pearsonr(dataset['SalePrice'],['residuals']))
#ven = dataset['SalePrice'] + ['residuals']
"""
scat1= seaborn.regplot(x="SalePrice",y="0",fit_reg=True, data=dataset,residuals)
plt.xlabel('SalePrice')
